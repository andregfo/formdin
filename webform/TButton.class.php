...
// 			$jsConfirm = 'if( !confirm("'.$this->getConfirmMessage().'")){return false;} ';
			$jsConfirmBegin = 'fwConfirm("'.htmlentities($this->getConfirmMessage()).'", function() { ';
...
// 				$this->addEvent('onclick',$jsConfirm.'if( typeof(btn'.ucwords($this->getId()).'OnClick)=="function"){btn'.ucwords($this->getId()).'OnClick(this);return true;};');
		...
		if( !$this->getEnabled() )
		{
			...
			$this->setProperty('title',htmlentities('Ação desabilitada') );
			// a imagem não tem como desabilitar então remover os eventos
			if( $isImage )
			{
				$this->clearEvents();
			}
