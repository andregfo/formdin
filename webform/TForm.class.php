...
class TForm Extends TBox
{
	...
	public function __construct( $strTitle=null, $strHeight=null, $strWidth=null, $strFormName=null, $strMethod=null, $strAction=null,$boolPublicMode=null )
	{
		...
		if( ! self::$instance )
		{
			...
			$this->setFade( 0 ); // habilitar o efeito fade-in para exibir o formulario
	   ...
	   if( $this->getFieldType() == 'form' )
	   {
		...
		if( $this->getShowCloseButton() )
		{
			...
			else
			{
				...
				{
					$this->headerCloseButton->add( '<img src="' . $this->getBase() . 'imagens/fwbtnclosered.jpg" style="cursor:pointer;float:right;width:28px; height:15px;vertical-align:top;margin-right:2px;" title="Fechar" onClick="fwConfirmCloseForm(\'' . $this->getId() . '\',' . $sub . ',' . $this->getOnClose() . ',' . $this->getOnBeforeClose().');">' );
				}
				else
				{
...
*/
			...
			foreach( $this->displayControls as $name=>$dc )
			{
				...
				$dcField 		= $dc->getField();
				$dcLabel		= $dc->getLabel();
				if( $dcLabel && !$dcLabel->getCss('font-size'))
				{
					$dcLabel->setCss('font-size',$this->getCss('font-size') );
				}
				if( $dcLabel && !$dcLabel->getCss('color'))
				{
				...
				{
					// ativar/desativar online doc nos forms das abas
			...
			else
			{
				if( $strPage === null )
				{
					...
					if( !in_array($field->getId(),$aIgnoreFields) && ( ! $aValidFields || in_array($field->getId(),$aValidFields) ) && method_exists( $field, 'validate' ) && !$field->validate() )
					{
						...
						if( $dc->getLabel() )
						{
							self::$errors[ $field->getId() ] = str_replace( "::", ":", $tab . $dc->getLabel()->getValue() . ':' . $field->getError() );
						}
						else
						{
...
*/
	...
	*/
	...
	{
		// incluir o campo dentro do grupo o da aba aberta
		if( is_array( $this->currentContainer ) && count( $this->currentContainer ) > 0 )
		{
	...
	/**
	 *  * Adicionar botão para requisição Ajax no formulário
	 *
	...
	public function addButtonAjax( $strValue=null, $strModule=null, $strJsBeforeSend=null, $strJsCallback=null, $strAction=null, $strMsgLoading=null, $strDataTypeReturn=null, $boolAsync=null, $strContainerId=null, $strName=null, $strConfirmMessage=null, $boolNewLine=null, $boolFooter=null, $strImage=null, $strImageDisabled=null, $strHint=null, $strVerticalAlign=null, $boolLabelAbove=null, $strLabel=null,$strHorizontalAlign=null )
	{
		...
		*/
		...
		$strOnClick = 'fwAjaxRequest( {"callback": (typeof ' . $function . ' == "function") ? ' . $function . ' : null,"beforeSend":' . ( $functionBeforeSend=='null' ? $functionBeforeSend : '"'.$functionBeforeSend.'"' ) . ',"action":"' . $strAction . '","async":' . ( ( $boolAsync ) ? 'true' : 'false' ) . ',"dataType":"' . $strDataTypeReturn . '","msgLoading":"' . $strMsgLoading . '","containerId":"' . $strContainerId . '","module":"' . $strModule . '"});';
	...
	*/
	 ...
	 * @return TCpfField
	 */
	public function addCpfField( $strName, $strLabel=null, $boolRequired=null, $strValue=null, $boolNewLine=null, $boolLabelAbove=null, $boolNoWrapLabel=null )
	{
	 ...
	 * @return TCnpjField
	 */
	public function addCnpjField( $strName, $strLabel=null, $boolRequired=null, $strValue=null, $boolNewLine=null, $boolLabelAbove=null, $boolNoWrapLabel=null )
	{
	 ...
	 */
	public function addCpfCnpjField( $strName, $strLabel=null, $boolRequired=null, $strValue=null, $boolNewLine=null, $boolLabelAbove=null, $boolNoWrapLabel=null )
	{
	 ...
	 * @return TMask
	 */
	public function addCepField( $strName, $strLabel=null, $boolRequired=null, $strValue=null, $boolNewLine=null, $strFieldEndereco=null, $strFieldBairro=null, $strFieldCidade=null, $strFieldCodidoUf=null, $strFieldSiglaUf=null, $strFieldNumero=null, $strFieldComplemento=null, $strFieldCodigoMunicipio=null, $boolLabelAbove=null, $boolNoWrapLabel=null,$jsCallback=null )
	{
		$field = new TMask( $strName, $strValue, '99.999-999', $boolRequired );
		$field->setFieldType( 'cep' );
		$field->addEvent( 'onBlur', 'fwValidarTamanhoCep(this)' );
		$arrFields = array( );
		...
		if( count( $arrFields ) > 0 )
		{
			...
			$button = new TButton( $field->getId() . '_btn_consultar', 'Consultar', null, 'getCepJquery("' . $field->getId() . '",' . json_encode( $arrFields ) . ',' . ($jsCallback ? $jsCallback : 'null').')', null, null, null, 'Infome o CEP e clique aqui para autocompletar os campos de endereço.' );
	 ...
	 * $strKeyField = nome da coluna que será utilizada para preencher os valores das opções
	 * $strDisplayField = nome da coluna que será utilizada para preencher as opções que serão exibidas para o usuário
	 *
	 * <code>
	 * 	// exemplos
	 ...
	 * @return TSelect
	 */
	public function addSelectField( $strName, $strLabel=null, $boolRequired=null, $mixOptions=null, $boolNewLine=null, $boolLabelAbove=null, $mixValue=null, $boolMultiSelect=null, $intSize=null, $intWidth=null, $strFirstOptionText=null, $strFirstOptionValue=null, $strKeyField=null, $strDisplayField=null, $boolNoWrapLabel=null )
	{
		...
		$field = new TSelect( $strName, $mixOptions, $mixValue, $boolRequired, $boolMultiSelect, $intSize, $intWidth, $strFirstOptionText, $strFirstOptionValue, $strKeyField, $strDisplayField );
		$this->addDisplayControl( new TDisplayControl( $strLabel, $field, $boolLabelAbove, $boolNewLine, $boolNoWrapLabel ) );
		return $field;
	}
	/**
	 ...
	 */
	public function addFileField( $strName, $strLabel=null, $boolRequired=null, $strAllowedFileTypes=null, $strMaxFileSize=null, $intFieldSize=null, $boolAsync=null, $boolNewLine=null, $strJsCallBack=null, $boolLabelAbove=null, $boolNoWrapLabel=null )
	{
		...
		{
			$field = new TFileAsync( $strName, $intFieldSize, $boolRequired, $strAllowedFileTypes, $strMaxFileSize, null, $strJsCallBack );
		}
		$this->addDisplayControl( new TDisplayControl( $strLabel, $field, $boolLabelAbove, $boolNewLine, $boolNoWrapLabel, null, null, null, true ) );
		return $field;
	}
	 ...
	 * @return TMask
	 */
	public function addMaskField( $strName, $strLabel=null, $boolRequired=null, $strMask=null, $boolNewLine=null, $strValue=null, $boolLabelAbove=null, $boolNoWrapLabel=null )
	{
