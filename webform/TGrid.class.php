...
class TGrid extends TTable
{
	...
	public function show( $boolPrint = true )
	{
		...
		else
		{
			...
			foreach( $this->getColumns() as $name => $objColumn )
			{
            	if( $objColumn->getVisible() )
                {
					...
					if ( $objColumn->getDataType() == 'checkbox' )
					{
						...
						{
							$tb = new TTable( 'chk_' . $objColumn->getId() . '_table' );
							$tb->setCss( 'font-size', '100%' );
							$tb->setCss( 'width', '100%' );
							$tb->setCss( 'background-color', 'transparent' );
							$tbr = $tb->addRow();
							...
							$chk->addEvent( 'onclick', "fwGridCheckUncheckAll(this,'{$objColumn->getEditName()}')" );
							...
							$cell = $tbr->addCell( $chk );
							$cell->setProperty( 'width', '10px' );
							$cell = $tbr->addCell( $objHeader->getValue() );
							$cell->setcss( 'text-align', 'center' );
							$tb = $tb->show( false ); // gerar o html sem enviar para o browser
							$objHeader->setValue( $tb );
						}
					}
					$row->add( $th );
				}
			...
			if ( is_array( $this->getData() ) )
			{
				...
				foreach( $res[ strtoupper( key( $res ) )] as $k => $v )
				{
					$rowNum++;
				...
				if ( $this->getExportExcel() )
				{
					...
					//$arrParams[ 'head' ] 	= json_encode( $this->getExcelHeadField() );
					$arrParams[ 'head' ] 	= NULL;
					...
					{
						$excel->setAttribute('title',htmlentities('Erro ao salvar os dados para exportação'));
					}
				}
	...
	*/
	...
	public function getActionColumnTitle( $strNewValue = null )
	{
		...
		{
			return htmlentities( 'Ação' );
		}
		return htmlentities( $this->actionColumnTitle );
	}
	...
	*/
	public function addSelectColumn( $strName, $strTitle = null, $strFieldName = null, $mixOptions = null, $strWidth = null, $boolReadOnly = null, $strFirstOptionText = null, $strFirstOptionValue = null, $strKeyField = null, $strDisplayField = null )
	{
		$col = new TGridSelectColumn( $strName, $strTitle, $strFieldName, $mixOptions, $strWidth, $boolReadOnly, $strFirstOptionText, $strFirstOptionValue, $strKeyField, $strDisplayField );
		$this->columns[ strtolower( $strName )] = $col;
		return $col;
	}
		...
		*/
		...
		if ( $this->numPages > 1 )
		{
			...
			foreach( $_REQUEST as $k => $v )
			{
				...
				{
					$arrRequest[ $k ] =$v;
					//echo addcslashes($v,"'").'<br>';
				}
			}
			$jsOnClick = isset( $jsOnClick ) ? $jsOnClick : '';
			if ( defined( APLICATIVO ) ) // já fez o includes dos arquivos ncessários
			{
				$arrRequest = preg_replace( '/"this.value"/', "this.value", json_encode( $arrRequest ) );
				$arrRequest = addcslashes($arrRequest,"'");
				//$select->setEvent('onChange',$jsOnClick.'jQuery("#'.$this->getId().'_table > tbody").load("'.$this->url.' tbody>tr",{ "ajax":1,"page":this.value});');
				$select->setEvent( 'onChange', $jsOnClick . 'jQuery("#' . $this->getId() . '_table > tbody").load("' . $this->url . ' tbody>tr",' . $arrRequest . ',function(){fwAttatchTooltip({"container":"'.$this->getId().'_table"});} )' );
				$select->setEvent( 'onKeyUp', 'this.onchange();' );
			}
			else
			{
				...
				$arrRequest = preg_replace( '/"this.value"/', "this.value", json_encode( $arrRequest ) );
				$arrRequest = addcslashes($arrRequest,"'");
				//$select->setEvent('onChange',$jsOnClick.'jQuery("#'.$this->getId().'_table > tbody").load(app_url+app_index_file+" tbody>tr",{ "ajax":1,"page":this.value,"modulo":"'.$this->url.'"});');
				$select->setEvent( 'onChange', $jsOnClick . 'jQuery("#' . $this->getId() . '_table > tbody").load(app_url+app_index_file+" tbody>tr",' . $arrRequest . ',function(){fwAttatchTooltip({"container":"'.$this->getId().'_table"});} )' );
				$select->setEvent( 'onKeyUp', 'this.onchange();' );
			}
		}
	...
	*/
	...
	public function autoCreateColumns()
	{
		...
		if ( $res )
		{
			$this->addKeyField( key( $res ) );
			$this->setUpdateFields( key( $res ) );
	...
	protected function configOffLine()
	{
		...
		else
		{
			if ( $res = $this->getData() )
			{
				$strFirstKeyField = strtoupper( key( $res ) );
		...
		if ( is_array( $frm->getDisplayControls() ) )
		{
			...
			foreach( $frm->getDisplayControls() as $fieldName => $dc )
			{
				...
				{
					$field->setAttribute( 'gridOfflineField', 'true' );
					$label = str_replace( ':', '', $dc->getLabel()->getValue() );
			...
			if ( $this->getUrl() )
			{
				...
				{
					$frm->addButton( 'Adicionar', null, 'btn' . $this->getId() . 'Save', 'jQuery("#' . $frm->getId() . '_footer").html(fw_img_processando2);jQuery("#' . $_POST[ 'parent_field' ] . '").load(app_url+app_index_file,{"ajax":0,"gridOffline":"1","modulo":"' . $this->getUrl() . '","parent_field":"' . $_POST[ 'parent_field' ] . '","action":"save","subform":1,' . $strJquery . '} );' );
				}
				$frm->addButton( 'Limpar', null, 'btn' . $this->getId() . 'Clear', 'jQuery("#' . $frm->getId() . '_footer").html(fw_img_processando2);jQuery("#' . $_POST[ 'parent_field' ] . '").load(app_url+app_index_file,{"ajax":0,"gridOffline":"1","modulo":"' . $this->getUrl() . '","parent_field":"' . $_POST[ 'parent_field' ] . '","action":"clear","subform":1,' . $strJquery . '} );' );
				$frm->addButton( 'Desfazer', null, 'btn' . $this->getId() . 'ClearAll', 'jQuery("#' . $frm->getId() . '_footer").html(fw_img_processando2);jQuery("#' . $_POST[ 'parent_field' ] . '").load(app_url+app_index_file,{"ajax":0,"gridOffline":"1","modulo":"' . $this->getUrl() . '","parent_field":"' . $_POST[ 'parent_field' ] . '","action":"clearAll","subform":1,' . $strJquery . '} );', 'Esta ação cancela todas as operações de inclusão, alteração e exclusão\nrealizadas no gride antes da última gravação.\n\n Confirma ?\n', null, null, null, null, 'Voltar os dados para a situação inicial' );
	...
	*/
	...
	//------------------------------------------------------------------------------------
	public function getExcelHeadField()
	{
		...
		foreach( $res[ key( $res )] as $k => $v )
		{
			foreach( $this->getColumns() as $name => $objColumn )
			{
				...
				if ( isset( $res[ $colName ] ) )
				{
					...
					{
						$colTitle = $objColumn->getTitle();
						$result[ $colTitle ][ $k ] = $res[ $colName ][ $k ];
					}
				}
			}
		}
